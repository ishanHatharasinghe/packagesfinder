...
const PackageFinder = () => {
  ...
  const fetchCSVData = () => {
    ...
    Promise.all([axios.get(prepaidUrl), axios.get(postpaidUrl)])
      .then(([prepaidResponse, postpaidResponse]) => {
        ...
        // Add a type identifier to each record
  ...
  const extractNumericPrice = (priceStr) => {
    ...
    // Remove 'Rs.' and any non-numeric characters except decimal points
    const numericStr = priceStr.replace(/[^\d.]/g, "");
    return parseFloat(numericStr) || 0;
  };

  const applyFilters = () => {
    // Get the correct dataset based on active tab
    const currentData = activeTab === "prepaid" ? prepaidData : postpaidData;

    // Start with all data for the current tab
    let filtered = [...currentData];

    // Log initial count
    console.log(`Initial ${activeTab} packages:`, filtered.length);
    ...
    // Filter by budget for both prepaid and postpaid
    if (budget) {
      filtered = filtered.filter((item) => {
        const priceField =
          activeTab === "prepaid" ? "Price(Rs:)" : "Monthly_Rental";
    ...
    if (packageTypes.length > 0) {
      ...
      console.log("After package type filter:", filtered.length);
    }

    // Filter by job field
    if (jobField) {
      filtered = filtered.filter((item) => {
        const itemJobField = item.job_field?.toLowerCase() || "";
        return itemJobField === jobField.toLowerCase();
      });
      console.log("After job field filter:", filtered.length);
    }

    // Filter by age range
    if (ageRange) {
      filtered = filtered.filter((item) => {
        const itemAgeRange = item.age_range || "";
        return itemAgeRange === ageRange;
      });
  ...
  const handleTabChange = (tab) => {
    ...
    setJobField("");
    setAgeRange("");
    // Set initial data for the selected tab
    setFilteredData(tab === "prepaid" ? prepaidData : postpaidData);
  };

  // Add debug output for data loading
  useEffect(() => {
      ...
      <div className="mb-6">
        ...
        <div className="mb-4">
          <h3 className="font-bold mb-2">
            Maximum {activeTab === "prepaid" ? "Budget" : "Monthly Rental"}{" "}
            (Rs.)
          </h3>
          <input
            type="number"
            ...
            placeholder={`Enter maximum ${
              activeTab === "prepaid" ? "budget" : "monthly rental"
            }`}
